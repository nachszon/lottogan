...
Relase Notes (03.11.2021)
Change the number display layout from vertical to horizontal
a) Output
b) File
...
void randomNumbersHandlerClass::m_create_random_numbers_array(int taking_number){
    ...
    time( & file_time );
    date_file = localtime( & file_time );

    strftime( str_date, 80, "%H_%M_%S", date_file );

    string file_name_1="numfile_";
    string file_name_2=str_date;
    string file_name_3=".txt";
    string file_name=file_name_1+file_name_2+file_name_3;
    ...
    file << "your lucky numbers:\n" << endl;

    for (int i=0;i<taking_number;i++){

        file << tab_rand_numb[i] <<" ";
...
/*--------------------------*/
    ...
    //show array with random numbers in output
    for (int i=0;i<taking_number;i++){
